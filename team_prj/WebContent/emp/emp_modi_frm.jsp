    ...
    <script type="text/javascript">
        $(function() {
        	$("#btnAcc").click(function(){
        		$("#empFrm").attr("action", "emp_acc_add.jsp");
			 	if(confirm("${param.empName}님을 추가 하시겠습니까?")){
					$("#empFrm").submit();
			 	}
			});
        	$("#btnDrop").click(function(){
        		$("#empFrm").attr("action", "emp_acc_del.jsp");
				if(confirm("${param.empName}님을 삭제 하시겠습니까?")){
					$("#empFrm").submit();
			}
			});
...
<form action=""id="empFrm"name="empFrm">
        ...
        List<PositionVO> positionlist = eDAO.positionList();
        for(int i=1;i < positionlist.size(); i++){
        %>
        <option<%=positionlist.get(i).getPosition().equals(position)?" selected='selected'":"" %>><%=positionlist.get(i).getPosition() %></option>
        <%	
        }
        %>
        </select>
